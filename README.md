...
- **Local Mode** (default, no AI account needed) — formatting/parseability checks (length, contact info detection, standard section headers, bullet points, layout artifacts, special characters) plus a keyword-overlap score: the most frequent meaningful terms from the pasted job posting, checked against which ones literally appear in the resume. This is a plain word-match heuristic (the same trick many rules-based ATS systems use), not an AI judgment of fit — no data ever leaves the browser, no API key, no backend, no cost. Works for every visitor, but results are limited to word matching: no relevance judgment, no rewrite suggestions.
